...
public class PaymentDelayPromptBuilder {
    ...
    private String buildSystemPrompt() {
        return """
                ...
                15. В требовании предупреди, что при неисполнении претензии кредитор вправе обратиться в суд; не выдумывай конкретный суд, если подсудность не передана.
                ...
                32. Банковские реквизиты указывай только дословно из case_facts.creditor.bank_details. Если поле пусто, не добавляй их.
                33. Сформируй раздел «Приложения» только из документов, существование которых подтверждено идентификатором документа во входных данных, и расчёта backend_calculation. Номер или дата сами по себе не подтверждают наличие файла.
    ...
    private String buildUserPrompt(GenerateClaimRequest request) {
        ...
        return """
                ...
                9. Исходная сумма обязательства, поступившие оплаты, остаток основного долга, неустойка и итог строго из backend_calculation.
                ...
                14. Раздел «Приложения» с нумерованным перечнем только реально подтверждённых входными данными документов. Расчёт задолженности включить обязательно. После требования укажи точные банковские реквизиты из case_facts.creditor.bank_details, если они переданы.
                ...
                1. CALCULATION включи всегда с required=true.
                2. CONTRACT включи только если заполнен case_facts.contract.document_id.
                3. ACT, TTN, INVOICE, TRANSPORT_ORDER и PAYMENT_EXTRACT не включай без отдельного идентификатора сохранённого документа во входных данных.
                5. document_name должен содержать фактический номер документа, если он известен.
                6. Каждый элемент attachments перечисли в разделе «Приложения» claim_text, не добавляя иных документов.
