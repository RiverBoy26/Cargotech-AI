...
public class PaymentDelayPromptBuilder {
    ...
    private String buildSystemPrompt() {
        return """
                ...
                33. Сформируй раздел «Приложения» только из документов, существование которых подтверждено входными фактами: договор, расчёт backend_calculation, акт, ТТН, счёт, транспортный заказ и подтверждённая бухгалтером выписка. Не выдумывай документы и реквизиты.
    ...
    private String buildUserPrompt(GenerateClaimRequest request) {
        ...
        return """
                ...
                14. Раздел «Приложения» с нумерованным перечнем только реально подтверждённых входными данными документов. Расчёт задолженности включить обязательно. Банковские реквизиты не добавлять, поскольку они отсутствуют во входных данных.
                ...
                2. CONTRACT включи, если переданы номер договора или contract_context.
                3. ACT, TTN, INVOICE и TRANSPORT_ORDER включай только при наличии соответствующего номера в shipment.
                4. PAYMENT_EXTRACT включай только при payment_confirmed_by_accountant=true.
