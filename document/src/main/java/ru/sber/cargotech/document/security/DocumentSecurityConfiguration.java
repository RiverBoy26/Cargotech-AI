...
@EnableConfigurationProperties(DocumentSecurityProperties.class)
public class DocumentSecurityConfiguration {
    ...
    @Bean
    JwtDecoder jwtDecoder(DocumentSecurityProperties properties) {
        String secretBase64 = properties.jwtSecretBase64();

        if (secretBase64 == null || secretBase64.isBlank()) {
            throw new IllegalStateException(
                "document.security.jwt-secret-base64 должен быть заполнен"
            );
        }

        byte[] secret;
        try {
            secret = Base64.getDecoder().decode(secretBase64.trim());
        } catch (IllegalArgumentException exception) {
            throw new IllegalStateException(
                "document.security.jwt-secret-base64 должен быть корректной Base64-строкой",
                exception
            );
        }
        ...
        if (properties.jwtIssuer() != null && !properties.jwtIssuer().isBlank()) {
