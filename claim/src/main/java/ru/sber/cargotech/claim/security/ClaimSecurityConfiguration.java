...
@EnableConfigurationProperties(ClaimSecurityProperties.class)
public class ClaimSecurityConfiguration {

    private static final String HMAC_ALGORITHM = "HmacSHA256";

    @Bean
    SecurityFilterChain claimSecurityFilterChain(
            HttpSecurity http,
    ...
    JwtDecoder jwtDecoder(
            ClaimSecurityProperties properties
    ) {
        String secretBase64 = properties.jwtSecretBase64();

        if (secretBase64 == null || secretBase64.isBlank()) {
            throw new IllegalStateException(
                    "claim.security.jwt-secret-base64 должен быть заполнен"
            );
        }
        ...
        try {
            ...
            throw new IllegalStateException(
                    "claim.security.jwt-secret-base64 должен быть корректной Base64-строкой",
                    exception
            );
        }
